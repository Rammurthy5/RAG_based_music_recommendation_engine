...
|---------|----------|------|------|
| `rag-service` | Python (FastAPI) | 8000 | RAG pipeline: embeddings, Weaviate retrieval, Gemini generation |
...
**Data flow:** Frontend → Go gateway (`/api/recommend`) → Python RAG service (`/recommend`) → Weaviate retrieval + Gemini generation → response back up the chain.
...
- **API keys required:** `GOOGLE_API_KEY`, `GENIUS_ACCESS_TOKEN` (MusicBrainz requires no API key, only a user-agent string).
...
- **Similarity threshold:** Default 0.25. Below threshold → fallback path.
